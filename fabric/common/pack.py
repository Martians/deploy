...
def download(c, name, source=None, local=None, path="/tmp"):
    ...
    """
    local = local if local else glob_conf['source']['source']
    ...
    """
    glob_conf['source']['source'] = file_path
...
def unpack(c, name, path=None, parent=None):
    parent = parent if parent else glob_conf['source']['parent']
    path = path if path else package()
