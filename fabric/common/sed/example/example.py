...
def kafka_config():
    index = 0
    host_port = '9092'
    zook_host = '{}:2128/kafka'.format(host.get_item(0, 'host'))
    ...
    sed.update(c, "log.dirs", host.get_item(0, 'disk', ','))
    sed.update(c, "broker.id", str(int(index) + 1))
    sed.update(c, "zookeeper.connect", zook_host)

    sed.enable(c, "advertised.listeners", 'PLAINTEXT://{}:{}'.format(host.get_item(index, 'host'), host_port))
